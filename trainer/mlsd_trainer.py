...
from loss.mlsd_loss import MLSDLoss
from utils.mlsd_decode import deccode_lines_TP_use_clsmap
from metric.mlsd_metric import F1_score_128, msTPFP, AP


class Simple_MLSD_Line_Learner():
    def __init__(self,
                 cfg,
        ...
        self.input_w = self.cfg.datasets.input_w
        self.input_h = self.cfg.datasets.input_h
        self.cls_num = self.cfg.model.num_classes

        self.loss_fn = MLSDLoss(cfg)
        self.epo = 0

    def step(self, step_n, batch_data: dict):
        imgs = batch_data["xs"].cuda()
        label = batch_data["ys"].cuda()
        # cls_label = batch_data['cls'].cuda()
        outputs = self.model(imgs)  # outputs(B, 40, 256, 256)
    ...
    def val(self, model, val_dataloader: DataLoader):
        ...
        down_scale = self.cfg.decode.down_scale
        ...
        for batch_data in data_iter:
            ...
            # keep TP mask
            # label = label[:, 19:, :, :]
            # batch_outputs = batch_outputs[:, 19:, :, :]
            ...
            for outputs, gt_lines_512 in zip(batch_outputs, batch_data["gt_lines_512"]):
                ...
                center_ptss, pred_lines, _, scores = \
                    deccode_lines_TP_use_clsmap(outputs, thresh, min_len, topk, 3, clsnum=self.cls_num)

                pred_lines = pred_lines.detach().cpu().numpy()  # (topk, 4)
                scores = scores.detach().cpu().numpy()  # (topk)

                pred_lines[: 0] = pred_lines[: 0] / (self.input_w / down_scale)
                pred_lines[: 2] = pred_lines[: 2] / (self.input_w / down_scale)

                pred_lines[: 1] = pred_lines[: 1] / (self.input_h / down_scale)
                pred_lines[: 3] = pred_lines[: 3] / (self.input_h / down_scale)

                pred_lines_128 = 128 * pred_lines

                gt_lines_512[: 0] = gt_lines_512[: 0] / self.input_w
                gt_lines_512[: 2] = gt_lines_512[: 2] / self.input_w

                gt_lines_512[: 1] = gt_lines_512[: 1] / self.input_h
                gt_lines_512[: 3] = gt_lines_512[: 3] / self.input_h
                gt_lines_128 = 128 * gt_lines_512
                ...
                if len(gt_lines_128) > 0:
                    tp, fp = msTPFP(pred_lines_128, gt_lines_128,
                                    sap_thresh)  # pred gt: (topk, 4) eg: (200, 4) True_positive and False positive np.zeros(len(predline))

                    n_gt += gt_lines_128.shape[0]  # gt的number
                    tp_list.append(tp)
                    fp_list.append(fp)
        ...
        precision = np.array(precisions, np.float32).mean()  # 求取平均值

        tp_list = np.concatenate(tp_list)
        fp_list = np.concatenate(fp_list)
        scores_list = np.concatenate(scores_list)
        idx = np.argsort(scores_list)[::-1]  # 从高往底找inx
        tp = np.cumsum(tp_list[idx]) / n_gt
        fp = np.cumsum(fp_list[idx]) / n_gt  # tp和fp占总共gt的比例
        rcs = tp
    ...
    def train(self, train_dataloader: DataLoader,
        ...
        for self.epo in range(epoches):
            ...
            for batch in data_iter:
                ...
                train_avg_class_loss.update(loss_dict['class_loss'].item(), 1)
                # train_avg_curve_seg_loss.update(loss_dict['curve_seg_loss'].item(), 1)
                # print("loss:", train_loss.item(), "avg:", train_avg_loss.avg, "c:", train_avg_center_loss.avg,
                #       "d:", train_avg_replacement_loss.avg, "l:", train_avg_line_seg_loss.avg, "junc:", train_avg_junc_seg_loss.avg,
                #       "junc:", train_avg_class_loss.avg,)

                status = '[{0}] lr= {1:.6f} loss= {2:.3f} avg = {3:.3f},c: {4:.3f}, d: {5:.3f}, l: {6:.3f}, ' \
                         'junc:{7:.3f},cls:{8:.3f}'.format(
                    self.epo + 1,
                    ...
                    train_avg_class_loss.avg,
                    # train_avg_curve_seg_loss.avg
                )
                # train_avg_match_loss.avg,
                # train_avg_match_rario.avg,m:{8:.3f},m_r:{9:.2f},
                # ,curve:{9:.3f}
            ...
            if self.epo > self.cfg.val.val_after_epoch:
                ...
                if best_score <= sAP10:
                    ...
                    early_n += 1
                self.logger.write(
                    "epo: {}, steps: {} ,fscore : {:.4f} , best fscore: {:.4f}, best recall: {:.4f}, best precision:{:.4f}". \
                    format(self.epo, self.global_step, sAP10, best_score, best_recall, best_precision))
